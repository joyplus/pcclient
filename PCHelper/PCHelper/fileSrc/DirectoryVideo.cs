...
{
    class DirectoryVideo : BindingList<FileVideo>
    {
        private FileVideo _directory;
        private bool _suspend = false;

        public DirectoryVideo() : this(Environment.GetFolderPath(Environment.SpecialFolder.Desktop)) { }

        private AsyncOperation _oper = null;

        public DirectoryVideo(string dir)
        {
            // Setup Async
        ...
        public void Activate(FileVideo fv)
        {
            if (fv.IsDirectory)
            {
                // Reload the list
        ...
        public FileVideo Find(string name)
        {
            FileVideo item = null;

            foreach (FileVideo fv in this.Items)
            {
                if (fv.Name == name)
                {
                    item = fv;
        ...
        {
            this.Add(new FileVideo(GetFileSystemInfo(e.FullPath)));
        }

        void FileSystem_Deleted(object sender, FileSystemEventArgs e)
        {
            FileVideo fv = this.Find(e.Name);
